...
#				else: song = ''
		...
		partymode = root.find('./Main_Zone/Basic_Status/Party_Info').text
		...
		xml_string = '<YAMAHA_AV cmd="GET"><Zone_2><Basic_Status>GetParam</Basic_Status></Zone_2></YAMAHA_AV>'
		Zone2 = xmitToReceiver(dev, xml_string)
		Z2power = Zone2.find('./Zone_2/Basic_Status/Power_Control/Power').text
		Z2volume = Zone2.find('./Zone_2/Basic_Status/Volume/Lvl/Val').text
		Z2dvolume = (str(float(volume) / 10)+' dB').replace('.', ',')
		Z2mute = Zone2.find('./Zone_2/Basic_Status/Volume/Mute').text
		Z2input = Zone2.find('./Zone_2/Basic_Status/Input/Input_Sel').text
		Z2inputname = Zone2.find('./Zone_2/Basic_Status/Input/Input_Sel_Item_Info/Title').text
		if (Z2input == 'SERVER' or Z2input == 'NET RADIO' or Z2input == 'USB' or Z2input == 'AirPlay'):
			if (Z2input == 'NET RADIO'):
				source = input.replace(' ', '_')
			else:
				source = input
				Z2inputname = Zone2.find('./Zone_2/Basic_Status/Input/Input_Sel_Item_Info/Title').text
		elif (input.find('AV') != -1):
			Z2dinput = Z2inputname+' - '+input
		elif (input.find('AUDIO') != -1):
			Z2dinput = Z2inputname+' - '+input
		else:
			Z2dinput = Z2inputname
		if (power != 'Standby'):
			...
			dev.updateStateOnServer('partymode', partymode)
			...
			dev.updateStateOnServer('partymode', 'Off')
		...
		if (Z2power != 'Standby'):
			dev.updateStateOnServer('Z2mute', mute)
			dev.updateStateOnServer('Z2input', inputmode)
		else:
			dev.updateStateOnServer('Z2mute', 'Off')
			dev.updateStateOnServer('Z2input', 'inputoff')
		dev.updateStateOnServer('Z2power', power)
		dev.updateStateOnServer('Z2volume', volume)
		dev.updateStateOnServer('Z2dvolume', dvolume)
	...
	def putPartymode(self, dev, val):
		...
		if val is None:
			self.debugLog(u'value not defined')
			return
		xml_string = '<YAMAHA_AV cmd="PUT"><System><Party_Mode><Mode>'+val+'</Mode></Party_Mode></System></YAMAHA_AV>'
		root = xmitToReceiver( dev, xml_string)
		self.updateStatus(dev)
	...
	def putZ2Mute(self, dev, val):
		...
		if val is None:
			self.debugLog(u'value not defined')
			return
		xml_string = '<YAMAHA_AV cmd="PUT"><Zone_2><Volume><Mute>'+val+'</Mute></Volume></Zone_2>></YAMAHA_AV>'
		root = xmitToReceiver( dev, xml_string)
		self.updateStatus(dev)

	def putZ2Volume(self, dev, val):
		if dev is None:
			self.debugLog(u'no device defined')
			return
		if val is None:
			self.debugLog(u'value not defined')
			return
		xml_string = '<YAMAHA_AV cmd="PUT"><Zone_2>><Volume><Lvl><Val>'+val+'</Val><Exp>1</Exp><Unit>dB</Unit></Lvl></Volume></Zone_2>></YAMAHA_AV>'
		root = xmitToReceiver( dev, xml_string)
		self.updateStatus(dev)

	def putZ2Power(self, dev, val):
		if dev is None:
			self.debugLog(u'no device defined')
			return
		if val is None:
			self.debugLog(u'value not defined')
			return
		xml_string = '<YAMAHA_AV cmd="PUT"><Zone_2><Power_Control><Power>'+val+'</Power></Power_Control></Zone_2></YAMAHA_AV>'
		root = xmitToReceiver( dev, xml_string)
		self.updateStatus(dev)

	def putZ2Input(self, dev, val):
		if dev is None:
			self.debugLog(u'no device defined')
			return
		if val is None:
			self.debugLog(u'value not defined')
			return
		xml_string = '<YAMAHA_AV cmd="PUT"><Zone_2>><Input><Input_Sel>'+val+'</Input_Sel></Input></Zone_2>></YAMAHA_AV>'
		root = xmitToReceiver( dev, xml_string)
		self.updateStatus(dev)
	...
	def setPartymode(self, pluginAction, dev):
		val = pluginAction.props['txtpartymode']
		self.putPartymode(dev, val)	
		
	def togglePartymode(self, pluginAction, dev):
		self.updateStatus(dev)
		val = 'On' if (dev.states['partymode']=='Off') else 'Off'
		self.putPartymode(dev, val)
	...
	def setZ2Mute(self, pluginAction, dev):
		val = pluginAction.props['ddlmute']
		self.putZ2Mute(dev, val)

	def toggleZ2Mute(self, pluginAction, dev):
		self.updateStatus(dev)
		val = 'On' if dev.states['mute']=='Off' else 'Off'
		self.putZ2Mute(dev, val)

	def setZ2Volume(self, pluginAction, dev):
		val = pluginAction.props['txtvolume']
		self.putZ2Volume(dev, val)

	def increaseZ2Volume(self, pluginAction, dev):
		self.updateStatus(dev)
		val = str(int(dev.states['volume']) + int(pluginAction.props['txtincrement']))
		self.putVZ2olume(dev, val)

	def decreaseZ2Volume(self, pluginAction, dev):
		self.updateStatus(dev)
		val = str(int(dev.states['volume']) - int(pluginAction.props['txtincrement']))
		self.putZ2Volume(dev, val)

	def setZ2Power(self, pluginAction, dev):
		val = pluginAction.props['ddlpower']
		self.putZ2Power(dev, val)

	def toggleZ2Power(self, pluginAction, dev):
		self.updateStatus(dev)
		val = 'On' if (dev.states['power']=='Standby') else 'Standby'
		self.putZ2Power(dev, val)

	def setZ2Input(self, pluginAction, dev):
		if (pluginAction.props['newinput'] == 'airplay'):
			val = 'AirPlay'
		else:
			val = pluginAction.props['newinput'].upper().replace('.','/').replace('_',' ')
		self.putZ2Input(dev, val)
